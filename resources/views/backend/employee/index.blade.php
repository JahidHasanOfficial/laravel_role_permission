                ...
                <tbody>
                    @forelse($datas as $key => $trainer)
                        <tr>
                            ...
                            <td class="px-4 py-2">{{ $trainer->address }}</td>
                            <td class="px-4 py-2"><img src="{{ App\Helpers\ImageHelper::get($trainer->image) }}"
                                    alt="Trainer Image" class="w-16 h-16 object-cover rounded"></td>
                            <td class="px-4 py-2 space-x-2">
                                <a href="{{ route('stuff.edit', $trainer->id) }}"
                                    class="bg-blue-500 text-white px-3 py-1 rounded">Edit</a>
                                <form action="{{ route('stuff.destroy', $trainer->id) }}" method="POST"
